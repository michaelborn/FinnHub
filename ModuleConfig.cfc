...
component {
	...
	// Module Entry Point
	this.entryPoint			= "FinnHub";
	// Inherit Entry Point
	this.inheritEntryPoint 	= false;
	// Model Namespace
	this.modelNamespace		= "FinnHub";
	// CF Mapping
	this.cfmapping			= "FinnHub";
	// Auto-map models
	...
	function configure(){
		...
		settings = {
			apiKey : getSystemSetting( "FINNHUB_API_KEY", "blablasecret" ),
			apiURL : getSystemSetting( "FINNHUB_API_URL", "https://finnhub.io/api/v1" ),
		};

		// Binder Mappings
		binder.map( "FinnHubClient" )
				.to( "hyper.models.HyperBuilder" )
				.asSingleton()
				.initWith({
					baseURL : settings.apiURL,
					headers = {
						"X-FinnHub-Token" : settings.apiKey
					}
				});
